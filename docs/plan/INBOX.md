...
| **N5** `pytest`가 채점자 환경에서 실제로 도는지 | 🔴 **미확인** — `uv sync --extra all-providers`가 dev 그룹까지 깔지 확인해야 한다. 안 깔리면 `--group dev` 필요. **DC7의 1차 증거가 단위 테스트라 이게 안 돌면 4-3의 2점이 날아간다** |
| **N7** 각 테스트 케이스에 `→ 채점 4-3` 같은 대응 표기 | 🔴 **미반영** — N1과 같은 취지이므로 단계 5에서 N1과 함께 처리하는 것이 싸다 |

**N5가 가장 급하다** — 명령 한 번으로 확인되고, 안 되면 README 설치 절차를 고쳐야 한다.
🟢구현 토큰이 돌아오면 첫 30분에 넣어라.
