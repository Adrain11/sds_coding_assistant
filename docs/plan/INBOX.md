...
→ **마감 (🔵검토, 09-17):** 세 건 전부 처리 확인.
- **R-A** `model_error`에 `status="error"` 추가 (`ac4988b`) — 회귀 테스트 2개 포함.
  README §6의 실제 출력에 모델 실패가 `>>` 마커로 잡힌 것으로 실물 검증까지 됐다.
- **R-B** trace 번호가 논리적 모델 호출을 가리키도록 수정 (`8b476e4`).
- **R-C** 👤사람이 TUI가 만든 실제 run으로 DC3·DC4·DC8 확인 → `step2_result.md`
  근거를 "TUI 실측"으로 교체 (`46a10f0`). `list`가 정상 동작해 S11 회귀도 없음이 확인됐다.
...
→ **마감 (🔵검토, 09-17):** P1~P4 전부 종결.
P1 `report.py` 작성 완료(`066a77c`) · P2 `atexit` 백스톱(`5177908`) ·
P3 `thread_id` 키잉으로 교체(`5177908`) · P4 T11-u 존재 확인 + `get_server_project_context()`
의존을 `STEP2_PLAN.md` §7에 기록. 단계 2는 DC1~DC8 전부 통과로 닫혔다.
...
→ **진행 (🔵검토, 09-17):** 9건 중 3건 완료, 3건은 단계 5로 이월, **3건은 아직 미확인.**

| | 상태 |
|---|---|
| **N2** 4절 `config path` 문구가 사실과 다름 | ✅ 반영 (`dff7088`) — `project hooks.json` 줄만 확인하도록 고치고 실제 출력 예시와 `(missing)` 설명을 붙였다 |
| **N3** `sds-assistant` 이름 확정 | ✅ 확정 (R4 결과) — 7절 구조·9절 문제해결 모두 일관 |
| **N6** 6절에 DC8 계층형 trace 출력 | ✅ 반영 (`4a79424`) — 실제 `report show` 출력을 붙였다 |
| **N1** 채점 16개 세부항목 ↔ 확인 방법 매핑 (`docs/evaluation-mapping.md`) | 📌 단계 5 이월 (WORKFLOW §0 기재) |
| **N8** "저장소 루트" → "압축을 푼 폴더의 최상위" | 📌 단계 5 이월 |
| **N9** 항목 2 테스트 케이스에 YOLO | 📌 단계 5 이월 — 단계 3 구현 후에야 쓸 수 있다. 검토 R6에 따라 `auto`(필수)+`yolo`(가능하면)로 나눈다 |
| **N4** "Python 3.12가 없어도 `uv`가 자동으로 받아온다" 한 줄 | ✅ **반영 (09-17)** — 1절 요구사항 표에 한 줄, 2절 설치 아래에 인용 블록. 전용 가상환경(`libs/code/.venv`)에 받고 시스템 Python은 안 건드린다는 점까지 명시 |
| **N5** `pytest`가 채점자 환경에서 실제로 도는지 | ✅ **확인 (👤사람, 09-17)** — `uv run --project libs/code pytest tests/ -q` → **43 passed.** 별도 `--group dev` 없이 돈다. README 6절 명령을 그대로 둔다.<br>⚠️ 단 이 환경은 `uv sync`를 여러 번 돌린 상태라 dev 의존성이 이미 깔려 있을 수 있다. **깨끗한 환경 검증은 단계 5의 ZIP 테스트(PLAN §단계5-5)에서 재확인한다** |
| **N7** 각 테스트 케이스에 `→ 채점 4-3` 같은 대응 표기 | 🔴 **미반영** — N1과 같은 취지이므로 단계 5에서 N1과 함께 처리하는 것이 싸다 |

남은 것은 **N7** 하나이고, N1(`docs/evaluation-mapping.md`)과 함께 단계 5에서 처리한다 — 같은 취지라 따로 하면 두 번 쓴다.
...
→ **마감 (🔵검토, 09-17):** 확인 완료 후 조치까지 끝났다.
`https://pypi.org/pypi/assistant/json` → **200** (v2.2.0a4, 릴리스 47개).
`requires-python`이 `>=3.8,<4`로 우리와 겹쳐 **조용히 설치되는** 최악의 형태였다.
→ 배포명을 `sds-assistant`로 변경(import 이름 `assistant` 유지), `libs/code/pyproject.toml`의
`dependencies`·`[tool.uv.sources]` 함께 수정 (`2e5499f`). `sds-assistant`는 PyPI에 없다(404).
