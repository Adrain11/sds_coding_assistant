...
## ✅ TUI 실측 결과 — MC1~MC7 **전부 통과** (2026.09.18, 👤사람)

절차: `docs/plan/tui_test_checklist.md`의 T1~T19를 EC1~EC9와 **한 세션에서 함께** 수행.
확인 주체는 👤사람(TUI 화면), 로그 대조는 🔵검토(`report show` 출력).

| MC | 확인 내용 | 결과 | 근거 |
|---|---|---|---|
| **MC1** | 세션1에서 가르친 규칙이 **새 세션**에 적용 | ✅ 통과 | T2 — 새 세션에서 "인사 함수 만들어줘" → 타입힌트가 붙어 나왔다. **더 강한 증거는 T11** — `search_memory`가 `[R6]`(T1에서 넣은 규칙)을 실제로 찾아 `memory_refs`에 넣었다 |
| **MC2** | `.deepagents/`의 메모리 파일이 세션 종료 후 잔존 | ✅ 통과 | `AGENTS.md`(`[R1]`~`[R6]`), `memories/lessons.md`(후보 기록) 모두 잔존 |
| **MC3** | `memory_refs` 없이 `create_plan` → 거부 | ✅ 통과 | T10 — 거부 + `search_memory`를 먼저 부르라는 안내 |
| **MC4** | 인용한 메모리가 `memory_hit`으로 로그에 남음 | ✅ 통과 | T19 — run `20260918-130249-01a0b2a8`에 `memory_hit  R1,R3,R5,R6  -  4 refs` |
| **MC5** | 실패·차단이 쌓인 뒤 개선안 생성 | ✅ 통과 (아래 §MC5 상세) | T16 |
| **MC6** | `verify_improvement`의 전후 비교 | ⚠️ **동작은 함. 숫자가 무의미하다** (아래 §MC6) | T17 — `before=7 after=8 → 통과` |
| **MC7** | TCB 경로를 대상으로 한 개선안 → 거부 | ✅ 통과 | T18 — 로그에 `improve_end tests/test_plan_gate.py ERROR ...TCB라 자기개선 대상이 될 수 없습니다` |

### MC5 상세 — 개선안이 **실제로 밟은 지뢰**에서 나왔다

`propose_improvement`가 `runs/*/events.jsonl`에서 찾은 반복 사유와 만든 후보:

| 후보 | 대상 | 반복 | 제안 요지 |
|---|---|---|---|
| `[L1]` | `.deepagents/AGENTS.md` | `"'execute'는 승인된 계획이 있어도 사용할 수 없습니다"` **4회** | `[R2]`에 "우회 시도도 하지 않는다 + 대안 경로(`write_file`)를 명시" 추가 — 4번이나 시도한 건 규칙이 금지만 적고 대안을 안 줬기 때문 |
| `[L2]` | `.deepagents/skills/plan-first/SKILL.md` | `"승인된 계획이 없습니다"` **3회** | "쓰기 도구 호출 전에 `get_plan_status`로 승인 상태를 먼저 확인하라" — 차단당한 뒤 반응하지 말고 사전 확인 |
| `[L1]`(2회차) | `.deepagents/AGENTS.md` | `"게이트 내부 오류로 차단됨"` **4회** | `[R7]` "게이트가 fail-closed로 막으면 게이트 자신의 결함이니 우회하지 말고 원인을 보고하고 기다린다. `plan_gate.py`는 TCB라 직접 못 고친다" |

**세 번째 후보가 가장 값어치 있다.** 근거가 된 `"게이트 내부 오류로 차단됨"` 4회는
**오늘 고친 S18/A2 버그가 남긴 `gate_block` 이벤트**다. 꾸며낸 사례가 아니라
이 프로젝트가 실제로 세 번 밟은 지뢰(S13 → EC6 → A2)에서 나온 개선안이고,
제안 내용도 실제로 맞다.

> 공식 3-3이 "**실행 실패·평가 결과를 바탕으로** 개선안을 생성"인데,
> 이보다 진짜인 사례를 만들 수 없다.

**모두 `pending`으로만 기록됐다** — M3(자동 적용 금지) 그대로다.

### ⚠️ MC6 상세 — 동작은 하지만 숫자가 3-4를 증명하지 못한다 (검토 A1)

실측 출력:

```
검증 결과 (candidate_id=L1): before=7 after=8 → 통과 — 사람이 승인하면 반영 가능합니다
```

`after = len(before_ids | new_ids)`이므로 **`before`보다 작아질 수 없다**(합집합).
`skills` 분기도 `(0,1)` 또는 `(1,1)`이라 마찬가지다. 따라서:

- §5 M4 4번 *"나빠졌으면 반영하지 않는다"*가 **구조적으로 발동 불가**
- §6 V6 *"after가 before보다 나쁘면 반영 안 함"*을 만족시키는 입력이 **존재하지 않음**
- `improved=False`의 뜻은 "나빠졌다"가 아니라 **"제안이 중복이다"**

지금 이 함수가 하는 일은 **중복 검사**다. 유용하지만 공식 3-4가 요구하는
"전후의 **효과**"도 "**기존 기능의 정상 동작**"도 아니다.

→ 🔵검토 A1로 보고됨. 회귀 검사(제안을 임시 적용한 뒤 기존 `[Rn]`이 사라지지 않았는지 —
`before_ids ⊆ after_ids`)로 바꾸면 `after < before`가 실제로 가능해지고,
그것이 "기존 기능의 정상 동작" 검증이 된다. 수정 예정.

### 🔴 부수 발견 — `improve_end`의 `status`/`error`가 `data` 안에 있어 `report fail`이 못 찾는다

T18의 run `20260918-140331-01a0b2a8` 출력:

```
run 20260918-140331-01a0b2a8   (23.5s, 실패 0)
├─ improve_end  tests/test_plan_gate.py   -   ERROR  '...'는 TCB라 자기개선 대상이 될 수 없습니다
지표  모델 2회(시도) · 도구 1회 · 재시도 0회 · 차단 0회 · 총 23.5s
```

**본문에는 ERROR 행이 있는데 헤더는 `실패 0`이다.**
`plan_gate.py:563`이 `data={"status": "error", "error": ...}`로 넣는데
`report.py`는 최상위 `e.get("status")`를 본다. 그래서:

| | 상태 |
|---|---|
`show` 본문 | ERROR 행 보임 (`_build_rows`가 `data`를 직접 읽음) |
`show` 헤더 · `stats` | **실패 0** — 틀림 |
**`report fail`** | **이 실패를 못 찾는다** — 채점 4-4 직격 |

검토 R-A(`model_error`에 `status` 누락)와 **같은 버그의 재발**이다.
`record()`에 `status`·`error` 최상위 인자가 이미 있으므로 그것을 쓰면 된다. 수정 예정.

### 남은 항목

| | 내용 |
|---|---|
| A1 | `verify_improvement`를 회귀 검사로 (위 §MC6) + `STEP4_PLAN.md` M4·MC6·§9 문구 |
| 신규 | `improve_end`의 `status`/`error`를 최상위로 (위 §부수 발견) + 회귀 테스트 |
